...
-- ═══════════════════════════════════════════════════════════════════
-- PLAYER WHITELIST
-- Hanya player dengan UserId yang terdaftar yang bisa menjalankan script
-- Tambahkan UserId kamu ke dalam tabel ALLOWED_PLAYER_IDS
-- ═══════════════════════════════════════════════════════════════════
...
-- Fungsi notifikasi mandiri (tidak butuh Main / Lib, langsung ke CoreGui)
local function showAccessDeniedGui(playerName, userId)
	pcall(function()
		...
		screenGui.Name             = "_DPP_AccessDenied"
		...
		panel.Position            = UDim2.new(0.5, 0, 0.4, 0) -- start sedikit ke atas
		...
		msg.Position              = UDim2.new(0, 10, 0, 78)
		msg.BackgroundTransparency = 1
		msg.Text                  = string.format("'%s' (Id: %d)\ntidak ada dalam whitelist.", playerName, userId)
		...
		TweenService:Create(panel,  TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
			{BackgroundTransparency = 0, Position = UDim2.new(0.5, 0, 0.5, 0)}):Play()
		...
		-- Animasi keluar & destroy setelah 4 detik
		task.delay(4, function()
			TweenService:Create(panel,  TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.In),
				{BackgroundTransparency = 1, Position = UDim2.new(0.5, 0, 0.45, 0)}):Play()
			...
			task.delay(0.5, function() pcall(function() screenGui:Destroy() end) end)
		end)
	end)
end
...
do
	...
	if not ok or not lp then
		warn("[Dex++] ❌ Tidak bisa mendapatkan LocalPlayer. Script dihentikan.")
		return
	end

	local allowed = false
	for _, id in ipairs(ALLOWED_PLAYER_IDS) do
	...
	if not allowed then
		warn(string.format("[Dex++] ❌ Akses ditolak! Player '%s' (UserId: %d) tidak ada di whitelist.", lp.Name, lp.UserId))
		showAccessDeniedGui(lp.Name, lp.UserId)
		return
	end

	print(string.format("[Dex++] ✅ Player '%s' (UserId: %d) diizinkan.", lp.Name, lp.UserId))
end
-- ═══════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════
-- DEX++ INSTANCE CHANGE SAVE/LOAD SYSTEM
-- Saves property changes made to game instances (Workspace, PlayerGui, etc.)
-- ═══════════════════════════════════════════════════════════════════

local DexSaveSystem = (function()
	local HttpService = game:GetService("HttpService")

	local SaveSystem = {}
	SaveSystem.FileName = "DexPlusPlus_InstanceChanges.json"
	SaveSystem.DebugMode = true

	-- changesMap[fullKey] = { Segments={...}, ClassName="", Props={ propName={t=,v=} } }
	SaveSystem.changesMap = {}

	-- changesList: ordered list of structural operations (add / delete / move)
	-- Each entry: { Action="add"|"delete"|"move", ... }
	SaveSystem.changesList = {}

	-- Get path segments of instance from game root
	local function getSegments(obj)
		local segs = {}
		local cur = obj
		for _ = 1, 60 do
			if not cur then break end
			local ok, par = pcall(function() return cur.Parent end)
			if not ok or par == nil then break end
			if par == game then
				local nOk, n = pcall(function() return cur.Name end)
				if nOk then table.insert(segs, 1, n) end
				break
			end
			local nOk, n = pcall(function() return cur.Name end)
			if not nOk then return nil end
			table.insert(segs, 1, n)
			cur = par
		end
		return #segs > 0 and segs or nil
	end

	-- Check if instance lives inside CoreGui (Dex's own UI)
	local function isInCoreGui(obj)
		...
		for _ = 1, 30 do
			...
			local sOk, svc = pcall(function() return game:GetService("CoreGui") end)
			if sOk and cur == svc then return true end
			cur = par
		end
		return false
	end

	-- Serialize a Roblox value to a JSON-safe table
	function SaveSystem:SerializeValue(typeName, val)
		if val == nil then return {t = typeName, v = nil} end
		local t, v = typeName, nil

		if t == "string" or t == "Content" then
			v = tostring(val)
		elseif t == "bool" or t == "boolean" then
			v = (val == true)
		elseif t == "int" or t == "int64" or t == "float" or t == "double" or t == "number" then
			v = tonumber(val) or 0
		elseif t == "Color3" then
			v = {r=math.floor(val.R*255+0.5), g=math.floor(val.G*255+0.5), b=math.floor(val.B*255+0.5)}
		elseif t == "BrickColor" then
			v = val.Number
		elseif t == "Vector2" then
			v = {x=val.X, y=val.Y}
		elseif t == "Vector3" then
			v = {x=val.X, y=val.Y, z=val.Z}
		elseif t == "UDim" then
			v = {s=val.Scale, o=val.Offset}
		elseif t == "UDim2" then
			v = {xs=val.X.Scale, xo=val.X.Offset, ys=val.Y.Scale, yo=val.Y.Offset}
		elseif t == "CFrame" then
			v = {val.X,val.Y,val.Z, val.R00,val.R01,val.R02, val.R10,val.R11,val.R12, val.R20,val.R21,val.R22}
		elseif t == "Rect" then
			v = {x0=val.Min.X, y0=val.Min.Y, x1=val.Max.X, y1=val.Max.Y}
		elseif t == "NumberRange" then
			v = {min=val.Min, max=val.Max}
		elseif t == "NumberSequence" then
			local kps = {}
			for _, kp in ipairs(val.Keypoints) do kps[#kps+1]={ti=kp.Time,vi=kp.Value,ei=kp.Envelope} end
			v = kps
		elseif t == "ColorSequence" then
			local kps = {}
			for _, kp in ipairs(val.Keypoints) do kps[#kps+1]={ti=kp.Time, r=math.floor(kp.Value.R*255+0.5), g=math.floor(kp.Value.G*255+0.5), b=math.floor(kp.Value.B*255+0.5)} end
			v = kps
		elseif t == "Ray" then
			v = {ox=val.Origin.X,oy=val.Origin.Y,oz=val.Origin.Z, dx=val.Direction.X,dy=val.Direction.Y,dz=val.Direction.Z}
		elseif t == "PhysicalProperties" then
			if val then v = {d=val.Density,f=val.Friction,e=val.Elasticity,fw=val.FrictionWeight,ew=val.ElasticityWeight} end
		elseif t == "EnumItem" then
			-- Store as full enum path e.g. "Enum.Material.SmoothPlastic"
			local ok2, str = pcall(tostring, val)
			v = ok2 and str or nil
		else
			-- Unknown type: try tostring fallback
			local ok2, str = pcall(tostring, val)
			v = ok2 and str or nil
		end
		return {t=t, v=v}
	...
	-- Deserialize a saved entry back to a Roblox value
	function SaveSystem:DeserializeValue(entry)
		local t, v = entry.t, entry.v
		if v == nil then return nil end

		if t == "string" or t == "Content" then return tostring(v)
		elseif t == "bool" or t == "boolean" then return (v == true)
		elseif t == "int" or t == "int64" or t == "float" or t == "double" or t == "number" then return tonumber(v)
		elseif t == "Color3" then return Color3.fromRGB(v.r,v.g,v.b)
		...
		elseif t == "UDim" then return UDim.new(v.s,v.o)
		elseif t == "UDim2" then return UDim2.new(v.xs,v.xo,v.ys,v.yo)
		elseif t == "CFrame" then return CFrame.new(v[1],v[2],v[3],v[4],v[5],v[6],v[7],v[8],v[9],v[10],v[11],v[12])
		elseif t == "Rect" then return Rect.new(v.x0,v.y0,v.x1,v.y1)
		elseif t == "NumberRange" then return NumberRange.new(v.min,v.max)
		elseif t == "NumberSequence" then
			local kps={}; for _,kp in ipairs(v) do kps[#kps+1]=NumberSequenceKeypoint.new(kp.ti,kp.vi,kp.ei) end
			return NumberSequence.new(kps)
		elseif t == "ColorSequence" then
			local kps={}; for _,kp in ipairs(v) do kps[#kps+1]=ColorSequenceKeypoint.new(kp.ti,Color3.fromRGB(kp.r,kp.g,kp.b)) end
			return ColorSequence.new(kps)
		elseif t == "Ray" then return Ray.new(Vector3.new(v.ox,v.oy,v.oz),Vector3.new(v.dx,v.dy,v.dz))
		elseif t == "PhysicalProperties" and v then return PhysicalProperties.new(v.d,v.f,v.e,v.fw,v.ew)
		elseif t == "EnumItem" then
			-- v is stored as "Enum.EnumType.ItemName" e.g. "Enum.Material.SmoothPlastic"
			local parts = tostring(v):split(".")
			if #parts == 3 then
				local ok2, enumVal = pcall(function() return Enum[parts[2]][parts[3]] end)
				if ok2 then return enumVal end
			end
			return nil
		end
		return nil
	...
	-- Record a property change on a game instance
	function SaveSystem:RecordChange(obj, propName, typeName, val)
		if isInCoreGui(obj) then return end
		local segs = getSegments(obj)
		if not segs then return end

		local fullKey = table.concat(segs, ".")
		if not self.changesMap[fullKey] then
			local cOk, cn = pcall(function() return obj.ClassName end)
			self.changesMap[fullKey] = {Segments=segs, ClassName=cOk and cn or "Unknown", Props={}}
		end
		self.changesMap[fullKey].Props[propName] = self:SerializeValue(typeName, val)

		if self.DebugMode then
			print(string.format("[Dex++] Tracked: %s.%s (%s)", fullKey, propName, typeName))
		end
	end

	-- Expose getSegments for external hooks (GROUP, UNGROUP, moveObject, etc.)
	SaveSystem._getSegments = getSegments

	-- Save all tracked changes to JSON file
	function SaveSystem:Save()
		if not writefile then warn("[Dex++] writefile not supported") return false, 0 end

		-- Prop changes
		local propList = {}
		for _, e in pairs(self.changesMap) do
			if next(e.Props) then
				propList[#propList+1] = {Segments=e.Segments, ClassName=e.ClassName, Props=e.Props}
			end
		end

		local totalCount = #propList + #self.changesList

		local ok, err = pcall(function()
			writefile(self.FileName, HttpService:JSONEncode({
				Version = "3.0",
				SavedAt = os.time(),
				StructuralChanges = self.changesList,  -- add / delete / move
				InstanceChanges = propList             -- property modifications
			}))
		end)

		if ok then
			if self.DebugMode then
				print(string.format("[Dex++] Saved: %d structural, %d prop change(s)", #self.changesList, #propList))
			end
	...
	-- Helper: resolve a segments path to an instance (returns nil if not found)
	-- Token table: resolved once per Load() call
	-- Tokens can appear anywhere in a Segments array
	-- Supported tokens:
	--   $LOCALPLAYER  -> name of LocalPlayer  (e.g. "lanaakanpro")
	--   $CHARACTER    -> same as $LOCALPLAYER (shorthand for Workspace character path)
	local function resolveTokens(segs)
		local ok, lpName = pcall(function()
			return game:GetService("Players").LocalPlayer.Name
		end)
		if not ok then lpName = nil end

		local resolved = {}
		for _, seg in ipairs(segs) do
			if (seg == "$LOCALPLAYER" or seg == "$CHARACTER") and lpName then
				resolved[#resolved+1] = lpName
			else
	...
	local function findBySegments(segs)
		if not segs or #segs == 0 then return nil end
		local resolved = resolveTokens(segs)
		local inst = game
		for _, seg in ipairs(resolved) do
			local cOk, child = pcall(function() return inst:FindFirstChild(seg) end)
			if not cOk or not child then return nil end
			inst = child
		end
		return inst ~= game and inst or nil
	end

	-- Load and apply saved changes from JSON file.
	-- DefaultConfig SELALU di-apply terlebih dahulu (persistent),
	-- kemudian user changes dari file di-apply di atasnya.
	-- Jika tidak ada save file, hanya DefaultConfig yang di-apply.
	function SaveSystem:Load()
		-- Selalu apply DefaultConfig dulu supaya tetap persistent
		do
			local dcOk, dcRestored, dcFailed = self:ApplyDefaultConfig()
			if self.DebugMode then
				print(string.format("[Dex++] DefaultConfig (persistent) applied: %d ok, %d failed/skipped", dcRestored or 0, dcFailed or 0))
			end
		end

		if not isfile or not isfile(self.FileName) then
			if self.DebugMode then
				print("[Dex++] No save file found, using DefaultConfig only.")
			end
			return true, 0, 0
		end
		...
		if not ok or not data or not data.InstanceChanges then
			warn("[Dex++] Failed to read save file")
			return false, 0, 0
		end

		local restored, failed = 0, 0

		-- ── Step 1: Structural changes (add / move / delete) ──────────────
		if data.StructuralChanges then
			for _, entry in ipairs(data.StructuralChanges) do
				local action = entry.Action

				if action == "add" then
					pcall(function()
						local parent = findBySegments(entry.ParentSegments)
						if parent then
							local newInst = Instance.new(entry.ClassName)
							if entry.Name then newInst.Name = entry.Name end
							newInst.Parent = parent
							restored = restored + 1
							if self.DebugMode then
								print("[Dex++] Added: " .. entry.ClassName .. " -> " .. table.concat(entry.ParentSegments, "."))
							end
						else
							failed = failed + 1
							if self.DebugMode then warn("[Dex++] Add failed - parent not found: " .. table.concat(entry.ParentSegments or {}, ".")) end
						end
					end)

				elseif action == "delete" then
					pcall(function()
						local inst = findBySegments(entry.Segments)
						if inst then
							inst:Destroy()
							restored = restored + 1
							if self.DebugMode then
								print("[Dex++] Deleted: " .. table.concat(entry.Segments, "."))
							end
						else
							failed = failed + 1
							if self.DebugMode then warn("[Dex++] Delete failed - not found: " .. table.concat(entry.Segments or {}, ".")) end
						end
					end)

				elseif action == "move" then
					pcall(function()
						local inst = findBySegments(entry.OldSegments)
						local newPar = findBySegments(entry.NewParentSegments)
						if inst and newPar then
							inst.Parent = newPar
							restored = restored + 1
							if self.DebugMode then
								print("[Dex++] Moved: " .. table.concat(entry.OldSegments, ".") .. " -> " .. table.concat(entry.NewParentSegments, "."))
							end
						else
							failed = failed + 1
							if self.DebugMode then warn("[Dex++] Move failed: " .. table.concat(entry.OldSegments or {}, ".")) end
						end
					end)
				end
			end
		...
		-- ── Step 2: Property modifications ────────────────────────────────
		for _, entry in ipairs(data.InstanceChanges) do
			local inst = findBySegments(entry.Segments)
			if inst then
				for propName, propEntry in pairs(entry.Props) do
					pcall(function()
						local val = self:DeserializeValue(propEntry)
						if val ~= nil then
							inst[propName] = val
						elseif propEntry.v ~= nil then
							pcall(function() inst[propName] = propEntry.v end)
						end
					end)
				end
				restored = restored + 1
				...
				failed = failed + 1
				if self.DebugMode then warn("[Dex++] Prop target not found: " .. table.concat(entry.Segments or {}, ".")) end
			end
		end

		if self.DebugMode then
			print(string.format("[Dex++] Load complete: %d ok, %d failed", restored, failed))
		end
		return true, restored, failed
	end

	-- Record a new instance being added to the game
	function SaveSystem:RecordAdd(obj)
		if isInCoreGui(obj) then return end
		local parentOk, par = pcall(function() return obj.Parent end)
		if not parentOk or not par then return end
		local parSegs = getSegments(par)
		if not parSegs then return end
		local classOk, cn = pcall(function() return obj.ClassName end)
		local nameOk, nm = pcall(function() return obj.Name end)
		table.insert(self.changesList, {
			Action = "add",
			ParentSegments = parSegs,
			ClassName = classOk and cn or "Instance",
			Name = nameOk and nm or nil
		})
		if self.DebugMode then
			print(string.format("[Dex++] Recorded ADD: %s (%s) -> %s", nm or "?", cn or "?", table.concat(parSegs,".")))
		end
	end

	-- Record an instance being deleted from the game
	function SaveSystem:RecordDelete(obj)
		if isInCoreGui(obj) then return end
		local segs = getSegments(obj)
		if not segs then return end
		local classOk, cn = pcall(function() return obj.ClassName end)
		table.insert(self.changesList, {
			Action = "delete",
			Segments = segs,
			ClassName = classOk and cn or "Unknown"
		})
		-- Remove from changesMap - no point modifying a deleted instance
		self.changesMap[table.concat(segs, ".")] = nil
		if self.DebugMode then
			print("[Dex++] Recorded DELETE: " .. table.concat(segs, "."))
		end
	end

	-- Record an instance being reparented (moved)
	-- oldSegs: segments of the instance BEFORE the move
	-- newParSegs: segments of the new parent AFTER the move
	function SaveSystem:RecordMove(obj, oldSegs, newParSegs)
		if isInCoreGui(obj) then return end
		if not oldSegs or not newParSegs then return end
		local classOk, cn = pcall(function() return obj.ClassName end)
		local nameOk, nm = pcall(function() return obj.Name end)
		table.insert(self.changesList, {
			Action = "move",
			OldSegments = oldSegs,
			NewParentSegments = newParSegs,
			ClassName = classOk and cn or "Unknown",
			Name = nameOk and nm or nil
		})
		-- Update changesMap key so prop changes still apply after the move
		local oldKey = table.concat(oldSegs, ".")
		if self.changesMap[oldKey] then
			local newSegs = {}
			for _, s in ipairs(newParSegs) do newSegs[#newSegs+1] = s end
			newSegs[#newSegs+1] = nameOk and nm or oldSegs[#oldSegs]
			local newKey = table.concat(newSegs, ".")
			self.changesMap[newKey] = self.changesMap[oldKey]
			self.changesMap[newKey].Segments = newSegs
			self.changesMap[oldKey] = nil
		end
		if self.DebugMode then
			print("[Dex++] Recorded MOVE: " .. table.concat(oldSegs,".") .. " -> " .. table.concat(newParSegs,"."))
		end
	end

	-- Clear tracked changes in memory (does not affect save file)
	function SaveSystem:ClearChanges()
		self.changesMap = {}
		self.changesList = {}
		if self.DebugMode then print("[Dex++] Cleared tracked changes") end
	end

	-- ═══════════════════════════════════════════════════════════════════
	-- DEFAULT CONFIG
	-- Applied automatically if no save file exists when Load() is called.
	-- Edit this table to set your personal defaults.
	-- Use $LOCALPLAYER / $CHARACTER as placeholders for the local player name.
	-- ═══════════════════════════════════════════════════════════════════
	SaveSystem.DefaultConfig = {
		StructuralChanges = {
			-- Example: add a TextLabel into PlayerGui on load
			-- { Action="add", ClassName="TextLabel", Name="MyLabel",
			--   ParentSegments={"Players","$LOCALPLAYER","PlayerGui","InGame","Menus"} },
		},

		InstanceChanges = {

			-- ── BillboardGui (rank tag above head) ───────────────────────
			{
				Segments  = {"Workspace","lanaakanpro","Head","Rank"},
				ClassName = "BillboardGui",
				Props = {
					LightInfluence = {t="double", v=0},
					StudsOffset    = {t="Vector3", v={x=0, y=2.5, z=0}},
					Brightness     = {t="double", v=0},
					AlwaysOnTop    = {t="bool",   v=true},
					Size           = {t="UDim2",  v={xs=0, xo=0, ys=0, yo=100}},
				},
			},

			-- ── UIScale inside rank BillboardGui ─────────────────────────
			{
				Segments  = {"Workspace","lanaakanpro","Head","Rank","UIScale"},
				ClassName = "UIScale",
				Props = {
					Scale = {t="double", v=1.25},
				},
			},

			-- ── Rank label (LocalPlayer character) ── Text: "- Scripter -"
			{
				Segments  = {"Workspace","$CHARACTER","Head","Rank","Rank2"},
				ClassName = "TextLabel",
				Props = {
					Visible = {t="bool",   v=true},
					Text    = {t="string", v="- Scripter -"},
					LocalizationMatchedSourceText = {t="string", v="- Scripter -"},
				},
			},

			-- ── Rank label (lanaakanpro fixed character) ── Text: "- Developer -"
			{
				Segments  = {"Workspace","lanaakanpro","Head","Rank","Rank2"},
				ClassName = "TextLabel",
				Props = {
					Visible = {t="bool",   v=true},
					Text    = {t="string", v="- Developer -"},
					LocalizationMatchedSourceText = {t="string", v="- Developer -"},
				},
			},

			-- ── Watermark TextLabel in PlayerGui ─────────────────────────
			{
				Segments  = {"Players","$LOCALPLAYER","PlayerGui","InGame","Menus","TextLabel"},
				ClassName = "Watermark",
				Props = {
					FontSize         = {t="EnumItem", v="Enum.FontSize.Size18"},
					TextTransparency = {t="double",   v=0.5},
					Text             = {t="string",   v="by Grimkithh"},
					AnchorPoint      = {t="Vector2",  v={x=0, y=1}},
					Font             = {t="EnumItem", v="Enum.Font.Gotham"},
					Position         = {t="UDim2",    v={xs=0, xo=0, ys=1, yo=0}},
					TextXAlignment   = {t="EnumItem", v="Enum.TextXAlignment.Left"},
					TextScaled       = {t="bool",     v=true},
					Size             = {t="UDim2",    v={xs=0.1, xo=0, ys=0.025, yo=0}},
				},
			},
	...
	-- Apply the DefaultConfig (same logic as Load but uses in-memory table)
	function SaveSystem:ApplyDefaultConfig()
		local data = self.DefaultConfig
		...
		if data.StructuralChanges then
			for _, entry in ipairs(data.StructuralChanges) do
				...
				elseif action == "delete" then
					pcall(function()
						local inst = findBySegments(entry.Segments)
						if inst then inst:Destroy() restored = restored + 1
				...
				elseif action == "move" then
					pcall(function()
						local inst   = findBySegments(entry.OldSegments)
						local newPar = findBySegments(entry.NewParentSegments)
						if inst and newPar then inst.Parent = newPar restored = restored + 1
						else failed = failed + 1 end
					end)
				end
			end
		...
		if self.DebugMode then
			print(string.format("[Dex++] DefaultConfig applied: %d ok, %d failed/skipped", restored, failed))
		end
		return true, restored, failed
	end
...
local function main()
	...
	moveObject = function(obj)
		...
		-- Dex++ Save System: record drag-drop move
		pcall(function()
			if DexSaveSystem and oldPar and newPar
				and oldPar.Obj and newPar.Obj
				and oldPar.Obj ~= game and newPar.Obj ~= game then
				-- Build old path from Explorer node hierarchy (before Roblox tree update)
				local oldSegs = {}
				local cur = node
				for _ = 1, 60 do
					if not cur or not cur.Obj or cur.Obj == game then break end
					local nOk, n = pcall(function() return cur.Obj.Name end)
					if not nOk then break end
					table.insert(oldSegs, 1, n)
					cur = cur.Parent
				end
				-- New parent path from actual Roblox hierarchy
				local newParSegs = DexSaveSystem._getSegments and DexSaveSystem._getSegments(newPar.Obj) or nil
				if #oldSegs > 0 and newParSegs and #newParSegs > 0 then
					DexSaveSystem:RecordMove(obj, oldSegs, newParSegs)
				end
			end
		end)
	...
	Explorer.InitRightClick = function()
		...
		context:Register("CUT",{Name = "Cut", IconMap = Explorer.MiscIcons, Icon = "Cut", DisabledIcon = "Cut_Disabled", Shortcut = "Ctrl+Z", OnClick = function()
			...
			for i = 1,#sList do
				...
				pcall(function() DexSaveSystem:RecordDelete(inst) end)
				pcall(destroy,inst)
			end
		...
		context:Register("PASTE",{Name = "Paste Into", IconMap = Explorer.MiscIcons, Icon = "Paste", DisabledIcon = "Paste_Disabled", Shortcut = "Ctrl+Shift+V", OnClick = function()
			...
			for i = 1,#sList do
				...
				for c = 1,#clipboard do
					...
					if cloned then
						cloned.Parent = inst
						pcall(function() DexSaveSystem:RecordAdd(cloned) end)
		...
		context:Register("DELETE",{Name = "Delete", IconMap = Explorer.MiscIcons, Icon = "Delete", DisabledIcon = "Delete_Disabled", Shortcut = "Del", OnClick = function()
			...
			local sList = selection.List
			for i = 1,#sList do
				pcall(function() DexSaveSystem:RecordDelete(sList[i].Obj) end)
		...
		context:Register("DELETE_CHILDREN",{Name = "Delete Children", IconMap = Explorer.MiscIcons, Icon = "Delete", DisabledIcon = "Delete_Disabled", Shortcut = "Shift+Del", OnClick = function()
			...
			for i = 1,#sList do
				pcall(function()
					for _, child in ipairs(sList[i].Obj:GetChildren()) do
						DexSaveSystem:RecordDelete(child)
					end
				end)
		...
		context:Register("GROUP",{Name = "Group", IconMap = Explorer.MiscIcons, Icon = "Group", DisabledIcon = "Group_Disabled", Shortcut = "Ctrl+G", OnClick = function()
			...
			pcall(function() DexSaveSystem:RecordAdd(model) end)
			for i = 1,#sList do
				pcall(function()
					local oldSegs = DexSaveSystem._getSegments and DexSaveSystem._getSegments(sList[i].Obj) or nil
					sList[i].Obj.Parent = model
					if oldSegs then
						local newParSegs = DexSaveSystem._getSegments(model)
						if newParSegs then DexSaveSystem:RecordMove(sList[i].Obj, oldSegs, newParSegs) end
					end
				end)
			end
		...
		context:Register("UNGROUP",{Name = "Ungroup", IconMap = Explorer.MiscIcons, Icon = "Ungroup", DisabledIcon = "Ungroup_Disabled", Shortcut = "Ctrl+U", OnClick = function()
			...
			local function ungroup(node)
				...
				for i = 1,#ch do
					pcall(function()
						local oldSegs = DexSaveSystem._getSegments and DexSaveSystem._getSegments(ch[i].Obj) or nil
						ch[i].Obj.Parent = par
						if oldSegs then
							local parSegs = DexSaveSystem._getSegments(par)
							if parSegs then DexSaveSystem:RecordMove(ch[i].Obj, oldSegs, parSegs) end
						end
					end)
				end
		...
		local function onClick(className)
			...
			for i = 1,#sList do
				...
				if success and obj then
					defaultProps(obj)
					pcall(function() DexSaveSystem:RecordAdd(obj) end)
				end
			end
		end
...
local function main()
	...
	Properties.SetProp = function(prop,val,noupdate,prevAttribute)
		...
		-- Dex++ Save System: record this property change on game instances
		pcall(function()
			local sList = Explorer.Selection.List
			for i = 1, #sList do
				local node = sList[i]
				local obj = node.Obj
				if obj and not prop.IsAttribute then
					-- Read back the ROOT property (e.g. Size as UDim2, not sub-prop float)
					local readOk, appliedVal = pcall(function() return obj[prop.Name] end)
					if readOk and appliedVal ~= nil then
						-- Use typeof() on actual value so sub-props (Size.X.Offset etc.)
						-- still serialize with the correct root type (UDim2, not float)
						local realType = typeof(appliedVal)
						local typeMap = {boolean="bool", number="double", string="string"}
						local typeName = typeMap[realType] or realType
						DexSaveSystem:RecordChange(obj, prop.Name, typeName, appliedVal)
					end
				end
			end
		end)
...
Main = (function()
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Developed by Chillz - Remake by Grimkithh",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.CreateSaveLoadUI = function(parentGui)
		-- Create Save/Load buttons for the main menu
		if not parentGui then
			warn("❌ [Dex++] CreateSaveLoadUI: parentGui is nil!")
			return
		end
		
		if not parentGui.OpenButton then
			warn("❌ [Dex++] CreateSaveLoadUI: OpenButton not found in parentGui!")
			return
		end
		
		print("🔧 [Dex++] Creating Save/Load UI...")
		local create = Main.Create
		
		local saveLoadButtons = create({
			{1,"Frame",{BackgroundTransparency=1,Name="SaveLoadButtons",Size=UDim2.new(0,110,0,30),Position=UDim2.new(1,-120,0,5),ZIndex=999}},
			{2,"TextButton",{BackgroundColor3=Color3.fromRGB(46,125,50),BorderSizePixel=0,Font=Enum.Font.SourceSansBold,Name="SaveBtn",Parent={1},Position=UDim2.new(0,0,0,0),Size=UDim2.new(0,50,0,28),Text="💾",TextColor3=Color3.fromRGB(255,255,255),TextSize=16,ZIndex=1000}},
			{3,"UICorner",{CornerRadius=UDim.new(0,4),Parent={2}}},
			{4,"TextButton",{BackgroundColor3=Color3.fromRGB(33,150,243),BorderSizePixel=0,Font=Enum.Font.SourceSansBold,Name="LoadBtn",Parent={1},Position=UDim2.new(0,60,0,0),Size=UDim2.new(0,50,0,28),Text="📁",TextColor3=Color3.fromRGB(255,255,255),TextSize=16,ZIndex=1000}},
			{5,"UICorner",{CornerRadius=UDim.new(0,4),Parent={4}}},
		})
		
		saveLoadButtons.Parent = parentGui.OpenButton
		
		local saveBtn = saveLoadButtons.SaveBtn
		local loadBtn = saveLoadButtons.LoadBtn
		
		-- Save Button Click Handler
		saveBtn.MouseButton1Click:Connect(function()
			local ok, savedCount = DexSaveSystem:Save()
			if ok and savedCount > 0 then
				saveBtn.Text = "✅"
				saveBtn.BackgroundColor3 = Color3.fromRGB(76,175,80)
			elseif ok then
				saveBtn.Text = "⚠️"
				saveBtn.BackgroundColor3 = Color3.fromRGB(255,152,0)
			else
				saveBtn.Text = "❌"
				saveBtn.BackgroundColor3 = Color3.fromRGB(244,67,54)
			end
			task.wait(1.5)
			saveBtn.Text = "💾"
		...
		loadBtn.MouseButton1Click:Connect(function()
			local ok, restoredCount, failedCount = DexSaveSystem:Load()
			if ok and restoredCount > 0 then
				loadBtn.Text = "✅"
				loadBtn.BackgroundColor3 = Color3.fromRGB(66,165,245)
			elseif not ok then
				loadBtn.Text = "❌"
				loadBtn.BackgroundColor3 = Color3.fromRGB(244,67,54)
			else
				loadBtn.Text = "⚠️"
				loadBtn.BackgroundColor3 = Color3.fromRGB(255,152,0)
			end
			task.wait(1.5)
			loadBtn.Text = "📁"
		...
		saveBtn.MouseEnter:Connect(function()
			if saveBtn.Text == "💾" then
				saveBtn.BackgroundColor3 = Color3.fromRGB(56,142,60)
			end
		end)
		...
		print("✅ [Dex++] Save/Load buttons created successfully!")
		print("💾 Save button: " .. tostring(saveBtn.Parent ~= nil and "Visible" or "Hidden"))
		print("📁 Load button: " .. tostring(loadBtn.Parent ~= nil and "Visible" or "Hidden"))
	...
	Main.CreateMainGui = function()
		...
		-- Create Save/Load Config Apps in AppsFrame
		Main.CreateApp({
			Name = "Save Config",
			...
			OnClick = function(callback)
				...
				spawn(function()
					-- Save all tracked instance property changes to JSON file
					local ok, savedCount = DexSaveSystem:Save()

					local color, msg
					if ok and savedCount > 0 then
						color = Color3.fromRGB(76,175,80)
						msg = "✅ Saved " .. savedCount .. " instance(s)!"
					elseif ok and savedCount == 0 then
						color = Color3.fromRGB(255,152,0)
						msg = "⚠️ Nothing tracked yet!\nEdit properties first."
					else
					...
					notif.Parent = Main.MainGui
					service.TweenService:Create(notif, TweenInfo.new(0.3), {Position = UDim2.new(0.5,-110,0,50)}):Play()
					task.wait(2.5)
					service.TweenService:Create(notif, TweenInfo.new(0.3), {Position=UDim2.new(0.5,-110,0,-70),BackgroundTransparency=1}):Play()
					service.TweenService:Create(label, TweenInfo.new(0.3), {TextTransparency=1}):Play()
					task.wait(0.3)
					notif:Destroy()
				end)
			end
		...
		Main.CreateApp({
			...
			OnClick = function(callback)
				if not callback then return end
				spawn(function()
					-- Load saved instance changes from file and apply them to game
					local ok, restoredCount, failedCount = DexSaveSystem:Load()
					...
					elseif restoredCount > 0 then
						...
						local extra = failedCount > 0 and (" (" .. failedCount .. " not found)") or ""
						msg = "✅ Loaded " .. restoredCount .. " instance(s)!" .. extra
					else
						color = Color3.fromRGB(255,152,0)
						msg = "⚠️ 0 instances loaded."
					end
		...
		-- Create Save/Load UI buttons (at top right corner of main button)
		spawn(function()
			wait(0.5) -- Wait for GUI to be fully initialized
			pcall(function()
				Main.CreateSaveLoadUI(Main.MainGui)
				if DexSaveSystem.DebugMode then
					print("✅ [Dex++] Save/Load UI initialized")
				end
			end)
		end)
	...
	Main.Init = function()
		...
		-- ═══════════════════════════════════════════════════════════════
		-- AUTO LOAD INSTANCE CONFIG
		-- Otomatis load DexSaveSystem (DefaultConfig + user changes) setelah GUI siap
		-- ═══════════════════════════════════════════════════════════════
		task.delay(2, function()
			pcall(function()
				print("[Dex++] 🔄 Auto-loading instance config...")
				local loadOk, restored, failed = DexSaveSystem:Load()
				if loadOk then
					print(string.format("[Dex++] ✅ Config berhasil dimuat: %d ok, %d gagal/skip", restored or 0, failed or 0))
				else
					warn("[Dex++] ⚠️ Gagal memuat config")
				end
			end)
		end)
		-- ═══════════════════════════════════════════════════════════════
